	...
	public void update() {
		if (0 >= applet.getHeight() || 0 >= applet.getWidth()) {
			...
			if (hoee > breite / 2) {
				...
				eckeOR.setPosition(0, (breite - hoee * 2) / 2 + hoee * 2);
				eckeUL.setPosition(hoee, (breite - hoee * 2) / 2);
				eckeUR.setPosition(hoee, (breite - hoee * 2) / 2);
				
				breiteSpielfeld = hoee * 2;
				ScreenG.setColor(Color.black);
				ScreenG.fillRect(0, 0, eckeOL.getY(), hoee);
				ScreenG.fillRect(eckeOR.getY(), 0, eckeOL.getY(), hoee);
			}
